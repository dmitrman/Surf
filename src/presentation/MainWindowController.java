...
public class MainWindowController implements Initializable {
	...
	void run(ActionEvent event) throws IOException {
		...
		final SurfCompare sc = new SurfCompare(ip.trancateAreaFromImage(
				carImage, 200, 200), emblemImage,bal,threshld,octav);
		// sc.display();
		// sc.matchesInfo();
		// Dialogs.showInformationDialog(stage,
		// "I have a great message for you!", "Information Dialog", "title");
		...
		if (sc.getResultOfMatching() > porog)
			out_str = "Авто и логотип совпадают";
		else
			out_str = "Авто и логотип не совпадают";
	...
	@FXML
	void thresholdClick(MouseEvent event) {
		threshld=Float.parseFloat(threshold.getText());
	}

	@FXML
	void octavesClick(MouseEvent event) {
		octav=Integer.parseInt(octaves.getText());
	}

	@FXML
	void balanceClick(MouseEvent event) {
